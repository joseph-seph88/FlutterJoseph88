...
class SearchContent extends ConsumerWidget {
  ...
  Widget _buildRecentSearchesHeader(BuildContext context, WidgetRef ref, String userId) {
    return Padding(
  ...
  Widget _buildRecentSearchesList(BuildContext context, WidgetRef ref, String userId) {
    // 중복 제거
  ...
  Future<void> _onClearAllRecentSearches(WidgetRef ref, String userId) async {
    try {
      ...
      ref.invalidate(recentSearchesProvider(userId));
    } catch (e) {
      debugPrint('최근 검색어 전체 삭제 실패: $e');
    }
  }

  Future<void> _onDeleteRecentSearch(WidgetRef ref, String userId, String search) async {
    try {
