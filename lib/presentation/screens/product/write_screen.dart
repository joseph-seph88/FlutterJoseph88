...
class _WriteScreenState extends ConsumerState<WriteScreen> {
  ...
  Future<void> _onSubmit() async {
    ...
    // 1. 필수 입력값 검증
    final images = ref.read(multiImageProvider);
    if (images.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      final imageFiles = selectedImages.map((state) => state.file).toList();
      debugPrint('Image Files Length: ${imageFiles.length}');
  ...
  String _extractDongName(String fullAddress) {
    final pattern = RegExp(r'([가-힣]+동|[가-힣]+읍|[가-힣]+면)');
    final match = pattern.firstMatch(fullAddress);
    return match?.group(0) ?? fullAddress;
  }
                      ...
                      itemBuilder: (context, index) {
                        if (index == 0) {
                          return GestureDetector(
                            onTap:
                                selectedImages.length < 10 ? _pickImage : null,
                            child: Container(
                              width: 88,
                              height: 88,
                              ...
                              Expanded(
                                child: Text(
                                  _selectedLocationName != null
                                      ? _extractDongName(_selectedLocationName!)
                                      : '장소를 선택해주세요',
                                  style: theme.textTheme.bodyMedium?.copyWith(
